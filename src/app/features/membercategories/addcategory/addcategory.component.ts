...
import { AddCategory } from '../../../shared/models/membertcategorymodels/member-category-add.models';
...
export class AddcategoryComponent {
  addMemberTypeForm: FormGroup;
    isLoading = false;
    private dialogRef = inject(MatDialogRef<AddcategoryComponent>); // Inject MatDialogRef correctly
    ...
    onSubmit(): void {
      ...
      this.isLoading = true;
      const newMemberType: AddCategory = {
        id: 0,
        name: this.addMemberTypeForm.value.name,
        nameAr: this.addMemberTypeForm.value.nameAr,
      };
  
      this.memberTypesService.addMemberCategory(newMemberType).subscribe({
        next: (response) => {
